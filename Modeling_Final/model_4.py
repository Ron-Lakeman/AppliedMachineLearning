...
def tune_with_grid(X_tr, y_tr, X_val, y_val, label_col, n_classes, balance_probabilities, param_grid):
    best_params = None
    best_auc = -np.inf
    best_acc = -np.inf
    best_f1 = -np.inf
    ...
    for params in ParameterGrid(param_grid):
        ...
        acc, auc, f1 = evaluate_model(clf, X_val, y_val, n_classes)
        if label_col == "Cover_Type":
            if f1 > best_f1:
                best_auc, best_acc, best_f1 = auc, acc, f1
                best_params = params
        else:   
        # Does it make more sense to use auc or acc as the deciding evaluation parameter? --> acc for binary, f1 for imbalanced multiclass 
            if (acc > best_acc) or (acc == best_acc and auc > best_auc):
                best_auc, best_acc, best_f1 = auc, acc, f1
                best_params = params
            print(f"Accuracy: {best_acc:.4f}")
            print(f"ROC AUC: {best_auc:.4f} (n_classes={n_classes})")
            print(f"F1: {best_f1:.4f} (n_classes={n_classes})")
    return best_params, best_acc, best_auc, best_f1
...
def evaluate_model(clf, X, y_true, n_classes, batch_size=2500):
    ...
    print(f"F1: {f1:.4f} (n_classes={n_classes})")
    return acc, auc, f1
...
def run_dataset_from_csv(
    ...
    best_params, best_acc, best_auc, best_f1 = tune_with_grid(
    ...
    print(f"Best val AUC: {best_auc:.4f}, Best val Acc: {best_acc:.4f},Best val F1: {best_f1:.4f} ")
    ...
    return clf, best_acc, best_auc, best_f1, test_predictions
...
if __name__ == "__main__":
    ...
    for cfg in datasets:
        ...
        clf, acc, auc, f1, test_predictions = run_dataset_from_csv(
    ...
    print("\nSummary of results (using validation metrics):")
